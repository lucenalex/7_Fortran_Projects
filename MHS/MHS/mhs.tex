...
\begin{figure}
	...
	\begin{tikzpicture}
		\begin{axis}[
			...
			height=0.6\textwidth,
			legend style={
				at={(0.98,0.02)}, % Posição da legenda (ajustado para o canto superior direito)
				anchor=south east, % Ancoragem no canto inferior direito
				legend cell align=left, % Alinhamento das entradas da legenda
				legend columns=2, % Número de colunas da legenda (ajustado de acordo com a quantidade de entradas)
				xshift=-10pt, % Deslocamento horizontal negativo
				yshift=10pt, % Deslocamento vertical positivo
			},
			]
			\addlegendentry{$\omega = 1.0$}
			\addplot [smooth, very thick, blue] table {dados_freq_1.dat};
