...
\begin{tikzpicture}
		\begin{axis}[
			...
			\addlegendentry{$\omega = 2.0$}
			\addplot [smooth, thick, green!50!black] table {dados_freq_2.dat};
			
			\addlegendentry{$\omega = 3.0$}
			\addplot [smooth, thick, yellow!80!black] table {dados_freq_3.dat};
			
			\addlegendentry{$\omega = 4.0$}
			\addplot [smooth, thick, orange] table {dados_freq_4.dat};
			
			\addlegendentry{$\omega = 5.0$}
			\addplot [smooth, thick, red] table {dados_freq_5.dat};
		\end{axis}
	\end{tikzpicture}
\caption{Gr\'{a}fico do Oscilador Harm\^{o}nico para Diferentes
Frequ\^{e}ncias Angulares.}%
